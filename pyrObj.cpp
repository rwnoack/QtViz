...
pyrObjElem::pyrObjElem(grObjElem* prev)
  :grObjElem(prev)
{
}

pyrObjElem* pyrObj::extendObjElem(void)
{
  grObjElem* prev = _el;
  pyrObjElem* new_el = new pyrObjElem(prev);
  _el = new_el;
  return new_el;
}

void pyrObjElem::draw(const grObj* parent)
{
  double v[3];
#if 0
  if (do_hidden_line_shift == true) {
    ...
    drw_with_hidden_line_shift(fil_pyr, el );
    /* restore the color */
  ...
  if (draw_filled) {
    std::cerr << "fix me filled pyrObj" << std::endl;
    throw "fix me";
  ...
  if (parent->SignNormal() > 0.0) {
    ...
    int i;
    std::cerr << "fix me pyrObj signNormal < 0" << std::endl;
    throw "fix me";
    i=0; v[0] = getX(i); v[1] = getY(i); v[2] = getZ(i); glVertex3dv(v);
    i=3; v[0] = getX(i); v[1] = getY(i); v[2] = getZ(i); glVertex3dv(v);
    i=2; v[0] = getX(i); v[1] = getY(i); v[2] = getZ(i); glVertex3dv(v);
